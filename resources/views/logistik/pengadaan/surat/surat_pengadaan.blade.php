...
@php
    ...
    $tanggal_permintaan = strftime('%d %B %Y', strtotime($pengadaan->tanggal));

    $harga_barang = [];

    foreach ($detilPengadaan as $key => $value) {
        array_push($harga_barang, $value->harga * $value->jumlah);
    }
@endphp
...
<body
    ...
    <table>
        <tr>
            <td style="padding-right: 100px">No. {{ $pengadaan->nomor_pengadaan }}</td>
            <td>&nbsp;</td>
    ...
    <div class="tanda_tangan_lembar_pertama">
        ...
        @if ($pengadaan->persetujuan == 0)
            <br>
            <br>
            <br>
            <br>
        @else
            <img src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->size(75)->generate($kepala[1]['name'])) !!} ">
        @endif
    ...
    <div class="detil_surat_lembar_kedua">
        <table>
            <tr>
                <td colspan="3">No. {{ $pengadaan->nomor_pengadaan }}</td>
            </tr>
            <tr>
                <td>Lampiran</td>
                <td>:</td>
            ...
            <tr>
                <td>Perihal</td>
                <td>:</td>
                <td><b><u>{{ $pengadaan->perihal }}</u></b></td>
            </tr>
        </table>
    </div>
    ...
    <div class="tanda_tangan_lembar_pertama">
        <p style="text-align: center">Mengetahui</p>
        <p style="text-align: center">Kepala {{ $kepala[0]['tingkat'] . ' ' . $kepala[0]['nama'] }}</p>
        @if ($pengadaan->persetujuan == 0)
            <br>
            <br>
            <br>
    ...
    <div class="tanda_tangan_lembar_pertama">
        <p style="text-align: center">Mengetahui</p>
        <p style="text-align: center">Kepala {{ $pemohon[0]['tingkat'] . ' ' . $pemohon[0]['nama'] }}</p>
        <img src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->size(75)->generate($pemohon[1]['name'])) !!} ">
        <p style="text-align: center">( {{ $pemohon[1]['name'] }} )</p>
    </div>
    ...
    <table>
        ...
        <tr>
            <td>No. {{ $pengadaan->nomor_pengadaan }}</td>
        </tr>
    </table>
    ...
    <table style="border: 1px solid black; width: 100%;border-collapse: collapse;">
        <thead style="border: 1px solid black;border-collapse: collapse">
            ...
            <th style="border: 1px solid black;border-collapse: collapse;">Merk</th>
        ...
        <tbody style="border: 1px solid black;border-collapse: collapse">
            @foreach ($detilPengadaan as $key => $value)
                <tr>
                    ...
                    <td style="border: 1px solid black;border-collapse: collapse;">
                        <center style="font-weight: bold">{{ $value->nama_barang }}</center>
                    </td>
                    <td style="border: 1px solid black;border-collapse: collapse;">
                        <center>{{ $value->merk }}</center>
                    </td>
                    <td style="border: 1px solid black;border-collapse: collapse;">
                        <center>{{ $value->jumlah }}</center>
                    </td>
                    <td style="border: 1px solid black;border-collapse: collapse;">
                        <center>Rp. {{ number_format($value->harga) }}</center>
                    </td>
                </tr>
            @endforeach
            <tr>
